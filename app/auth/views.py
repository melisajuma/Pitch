...
def register():
    ...
    if form.validate_on_submit():
        user = User_pitch(email=form.email.data,username=form.username.data,password=form.password.data)
        db.session.add(user)
        db.session.commit()
...
def login():
    ...
    if login_form.validate_on_submit():
        user=User_pitch.query.filter_by(email=login_form.email.data).first()
